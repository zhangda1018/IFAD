...
from lib.backbone.metrics import ArcMarginProduct
...
class Network(nn.Module):
    def __init__(self, cfg, mode="train", num_classes=1000):
        ...
        self.classifier_a = self._get_classifer()
        self.classifier_b = self._get_classifer()
    ...
    def forward(self, x, **kwargs):
        if "feature_flag" in kwargs or "feature_cb" in kwargs or "feature_rb" in kwargs:
            ...
            if "classifier_a" in kwargs:
                return self.classifier_a(x, kwargs["label"], kwargs["device"])
            elif "classifier_b" in kwargs:
                return self.classifier_b(x, kwargs["label"], kwargs["device"])
        ...
        # x = self.classifier(x)
        x= self.classifier_a(x)+self.classifier_b(x)
        return x
    ...
    def _get_classifer(self):
        ...
        if self.cfg.CLASSIFIER.TYPE == "FCNorm":
            ...
            classifier = ArcMarginProduct(in_features=2048, out_features=self.num_classes, s=30, m=0.1, easy_margin=False)
        else:
            raise NotImplementedError
