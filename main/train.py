# import os
# os.sys.path.append('/data/zhangda/BBN-reweighting-code/BBN-reweighting-code/')
...
from lib.core.function import train_model, valid_model, sample_model, valid_model_with_f1
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=False,
        default="/data/zhangda/TScode/IFAD/configs/Flight.yaml",
        type=str,
    )
    parser.add_argument(
        "--ar",
...
def train(device_id):
    ...
    sample_set = Sample_Weight("sample", cfg)
    ...
    combiner = Combiner(cfg, device, num_classes)
    ...
    for epoch in range(start_epoch, epoch_number + 1):
        ...
        if cfg.VALID_STEP != -1 and epoch % cfg.VALID_STEP == 0:
            ...
            # )
            valid_acc, valid_loss, f1_score = valid_model_with_f1(
                validLoader, epoch, model, cfg, criterion, logger, device, writer=writer
            )
            ...
            sample_dict, out_0, out_1 = sample_model(
                sampleLoader, epoch, model, cfg, criterion, logger, device, writer=writer
            )
            train_set.update_sample_weight(sample_dict=sample_dict)
            train_set.update_epoch(epoch)
            combiner.update_weight(out_0, out_1)
